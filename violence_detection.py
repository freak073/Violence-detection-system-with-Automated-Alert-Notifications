...
def display_video_with_classification(video_path, model):
    global violence_counter, violence_detected, last_notification_time
    
    # Reset counters at the start of each video
    violence_counter = 0
    violence_detected = False
    ...
    while cap.isOpened():
        ...
        if frame_class == "Violence":
            ...
            violence_counter = 0
            violence_detected = False  # Reset violence_detected when no violence is detected

        current_time = time.time()
        # Trigger action if violence detected for consecutive frames and enough time has passed since last notification
        if violence_counter >= 10 and (current_time - last_notification_time) >= NOTIFICATION_COOLDOWN:
